      ...
      <div className="form-row">
        <div className="field" style={{ flex: 1, minWidth: 200 }}>
          ...
          <input
            ...
            placeholder={t("summarySection.requestNumberPlaceholder")}
          />
        </div>
        <div className="radio-row">
          ...
          {countsFor[storageId] != null && <b>({countsFor[storageId]})</b>}
        </div>
      </div>
      {request && (
        <div className="info-bar" style={{ marginTop: 10 }}>
          <span>
            {t("summarySection.request")}: <b>{request.req_id ?? request.request_id}</b>
          </span>
          <span>
          ...
          <span>
            {t("summarySection.savedDevices")}: <b>{currentDevices.length}</b>
          </span>
          <span>
            {t("summarySection.balance")}: <b>{request.financial_deducted ? t("summarySection.deducted") : t("summarySection.notDeducted")}</b>
          </span>
        </div>
      )}
      {request && (
